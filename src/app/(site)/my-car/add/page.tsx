...
export default function AddCar() {
  ...
  const getColorOptions = async () => {
    try {
      if (context.token) {
        setIsLoading(true);
        const response = await get(`color`);
        const data = response.data?.data as Color[];
        setColorOptions(
      ...
      toastError("Get colors data failed");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const getTypeOptions = async () => {
    try {
      if (context.token && brand) {
        // setIsLoading(true);
        const response = await get(
          `cartype?brand_id=${brand}`
        );
        const data = response.data?.data as CarType[];
        setTypeOptions(
          data.map((item) => ({
            label: item.name,
            value: item.name,
      ...
      toastError("Get car types data failed");
    } finally {
      setIsLoading(false);
    }
  };
